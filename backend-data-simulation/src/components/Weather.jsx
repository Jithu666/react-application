  ...
  return (
    <div>
      {/* <ul>
        {data.map((weather) => (
          <div>
            <li key={weather.id}>{JSON.stringify(data)}</li>
            <hr />
          </div>
        ))}
      </ul> */}
      ...
      {loading ? (
        ...
        <div className="flex flex-col justify-center">
          <h1
           ...
           justify-center text-2xl underline m-6"
          >
            Weather Information for {data.timezone}
          </h1>
          <div className="flex flex-col justify-center items-center">
            <div className="flex flex-col items-left p-4 border w-fit rounded text-lg">
              <p>Location: {data.timezone}</p>
              <p>Latittude: {data.latitude}</p>
